...
Dazu gibt es einen \ttt{vector} von \ttt{list<Rec*>} der Größe \ttt{M} namens \ttt{placedRectangles},
...
Dann wird es für \ttt{r} nach einer Lücke in \ttt{placedRectangles} gesucht.
...
das Rechteck \ttt{r} wird in alle Streifen in \ttt{placedRectangles} eingefügt, 
zu denen es gehört.
...
in allen Listen \ttt{placedRectangles}. 
...
Es wird eine Kopie des ganzen \ttt{vector} \ttt{placedRectangles} gemacht
...
Alle kollidierenden Rechtecke werden in eine Liste \ttt{rec\_remove}
hinzugefügt. 
Danach werden die Werte \ttt{x1} und \ttt{x2} aller Rechtecke, die zu \ttt{rec\_remove}
gehören, zu $-1$ gesetzt.
Als Nächstes werden alle Rechtecke, deren Koordinaten $-1$ betragen, aus \ttt{placedRectangles}
entfernt.
Danach wird die Funktion \ttt{addNew} ausgeführt, die das Rechteck \ttt{rr} und ggf.
die neuen anderen Rechtecke in \ttt{placedRectangles} einfügt.
...
\ttt{placedRectangles} wird zu \ttt{oldPlaced} gesetzt, die Funktion gibt den Wert \ttt{false} 
aus und bricht ab.

Die Methode \ttt{addNew()} nimmt als Argument das Rechteck zum Einfügen in eine Lücke.
Es wird in dieser Funktion in \ttt{placedRectangles} an der richtigen Stelle platziert.
Dann lässt man die Funktion \ttt{processStripeReturn()},
die probiert, neue Rechtecke in \ttt{placedRectangles} zu platzieren, für jeden Streifen laufen. 
Die Methode \ttt{placedRectangles} funktioniert ähnlich wie die Methode \ttt{processStripe}.
...
in \ttt{placedRectangles} gesucht.
Wenn es so eine Lücke gibt, wird geprüft, ob die Lücken in allen Streifen in \ttt{placedRectangles}
bestehen, zu denen das iterierte Rechteck gehört.
Wenn ja, wird dieses Rechteck in alle diesen Streifen in \ttt{placedRectangles} eingefügt.
