...
die Sortierkriterien \texttt{grea\-ter\-Ho\-les\-Si\-ze} und \texttt{smallerSize} zu wählen.
...
Die Ergebnisse mit den zufälligen Reihenfolgen werden im Folgenden beschrieben.

Anstatt dass die obengenannten Sortierkriterien verwendet werden, um die Liste $H$ und die Listen $U_j$
zu sortieren, kann man die Lücken und die Rechtecke in diesen Listen zufällig ordnen.
Dazu wurde der eingebaute Zufallszahlengenerator \ttt{random\_device} und die Funktion
\ttt{shuffle} verwendet. 
In der \cref{tab:random} sind die Ergebnisse zu den Beispielen 5 und 7 zu sehen.
Wie man feststellen kann, sind die Maxima aus allen Kombinationen nicht größer
als die Maxima in der \cref{tab:ergebnisse}, aber die Minima sind 
nicht größer als die Minima der Ergbenisse ohne Zufallszahlengenerator.
Dank der Zufälligkeit kann man zum Beispiel 7 ein um 12 besseres oder ein um 11 schlechteres
Ergebnis im Vergleich zur Kombination \ttt{greaterHolesSize}---\ttt{smallerSize} erzielen.
Das Ergebnis Zum Beispiel 5 kann dank der zufälligen Reihenfolge nicht verbessert werden,
aber kann im schlimmsten Fall um 193, also um 1,8 Prozentpunkte, verschlechtert werden.
Die mittels des Zufallszahlengenerators generierten Ergebnisse zu den anderen Beispielen
sind dieselben wie in der \cref{tab:ergebnisse}.
